...
return the number of players placed on IL since 1951
```{r}
oldInjuries <- oldInjuries %>%
  filter(grepl("Placed on IL", Notes)
```
...
oldInjuries <-
  oldInjuries %>%
  group_by(Team) %>%
  mutate( = mean(Longitude), Latitude = mean(Latitude)) %>% # get the average location
...
This graph shows the number of injuries sustained by each NBA team
```{r}
injuries %>%
  filter(!is.na(Team)) %>%
  count(Team) %>%
